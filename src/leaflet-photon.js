...
L.PhotonSearch = L.PhotonBase.extend({
    ...
    _onSelected: function (feature) {
        this.map.setView([feature.geometry.coordinates[1], feature.geometry.coordinates[0]], this.map.DEFAULT_ZOOM);
        app.localisation.showLocation.call(app.localisation,L.latLng([feature.geometry.coordinates[1], feature.geometry.coordinates[0]]),false,false,1);
        $(".photon-input").hide();
    },
    ...
    handleResults: function(geojson) {
        ...
        this.forEach(geojson.features, function (feature) {
            ...
            if(maxBounds.contains(L.latLng(feature.geometry.coordinates[1],feature.geometry.coordinates[0])))
            {
    ...
    getParams: function () {
        return {
            q: this.CACHE + " Ille-et-Vilaine, Bretagne, France",
